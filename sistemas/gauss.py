...
def executar_gauss():
    ...
    for i in range(n):
        while True:
            try:
                ...
                if len(entrada) != n + 1:
                    print(f"Erro: Você deve fornecer {n+1} valores (n coeficientes + 1 termo independente). Tente novamente.")
                    continue
    ...
    try:
        solucoes = gauss_eliminacao(A, b, n)
...
def gauss_eliminacao(A, b, n):
    ...
    for i in range(n - 1, -1, -1):
        solucoes[i] = matriz_aumentada[i, n]
        
        for j in range(i + 1, n):
            solucoes[i] = solucoes[i] - matriz_aumentada[i, j] * solucoes[j]
        
        solucoes[i] = solucoes[i] / matriz_aumentada[i, i]
